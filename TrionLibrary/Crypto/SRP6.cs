...
namespace TrionLibrary.Crypto
{
    ...
    //    public SRP6()
    //    {
    //        s = Array.Empty<byte>().GenerateRandomKey(SaltLength);
    //        _used = true;
    //    }
    ...
    //    {
    //        BigInteger b = new(Array.Empty<byte>().GenerateRandomKey((int)N.GetBitLength()), true);
    //        b %= (N - 1);
    //        return b;
    //    }
    ...
    //        BigInteger ourM = new(_sha1.ComputeHash(NgHash.Combine(I.ToByteArray().Combine(s.Combine(A.ToByteArray().Combine(B.ToByteArray().Combine(K)))))), true, true);
    //        if (ourM == clientM1)
    ...
    //        Rfc2898DeriveBytes rfc = new(tmp, salt, (int)GetXIterations(), HashAlgorithmName.SHA512);
